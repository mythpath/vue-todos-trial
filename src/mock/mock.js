/*
 * @Author: Bill Lee
 * @Date: 2020-04-15 18:02:27
 * @LastEditors: Bill Lee
 * @LastEditTime: 2020-04-18 16:08:46
 * @Description:
 */
  ...
  start () {
    ...
    mock.onGet('/todo/list').reply(config => { // config指前台传过来的值
      let mockTodo = Todos.map(tode => { // 重组Todos数组，变成我们想要的数据
        return {
          ...
          count: tode.record.filter((data) => {
            ...
            return false
          }).length, // 过滤掉record里面'checked'为true的数据，因为它们已经被完成了
          locked: tode.locked,
          isDelete: tode.isDelete
        }
      }).filter(tode => {
        if (tode.isDelete === true) {
          return false
      ...
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          resolve([200, {
            todos: mockTodo // 返回状态为200，并且返回todos的数据
          }])
        }, 200)
      })
    })
    ...
    mock.onGet('/todo/listId').reply(config => {
      ...
      return new Promise((resolve, reject) => {
        setTimeout(() => { resolve([200, {todo: todo}]) }, 200)
      })
    })
